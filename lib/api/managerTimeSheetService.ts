...
export const managerTimeSheetService = {
  ...
  async getEmployeeTimesheets(employeeId: string, page = 0, size = 25): Promise<WebResponseDTOListTimeSheetResponseDto> {
    const res = await api.get("/manager/employee/timesheet/list", {
      params: { employeeId, page, size },
    });
    return res.data;
  },

  async approveTimesheets(timesheetIds: string[]): Promise<WebResponseDTOString> {
  ...
  async rejectTimesheets(timesheetIds: string[]): Promise<WebResponseDTOString> {
